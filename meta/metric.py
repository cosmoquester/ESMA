...
def meta_wrong_yes(correctness: list[int], yes: list[int]) -> list[int]:
    return [1 - correct for correct, yes in zip(correctness, yes) if yes == 1]


def meta_wrong_no(correctness: list[int], yes: list[int]) -> list[int]:
...
def meta_metrics(
    direct_outputs: list[str], meta_outputs: list[str], answer_lists: list[list[str]]
) -> tuple[list[int], list[int], list[int], list[int], list[int]]:
    direct_correctness = correctness_by_inclusion(direct_outputs, answer_lists)
    yes = meta_yes(meta_outputs)
    yes_failures = meta_wrong_yes(direct_correctness, yes)
    no_failures = meta_wrong_no(direct_correctness, yes)
